...
use crate::types::{MempoolEvent, BlockHeader};
...
impl EventLoop {
    ...
    async fn connect_and_subscribe(&self) -> Result<()> {
        ...
        write.send(Message::Binary(pending_bytes))
            .await
        ...
        write.send(Message::Binary(blocks_bytes))
            .await
            .map_err(|e| ArbitrageError::WebSocket(format!("Send failed: {}", e)))?;
    ...
    async fn handle_pending_tx(&self, tx: &serde_json::Value) {
        ...
        // Extract gas price
        let gas_price_hex = tx.get("gasPrice")
            .and_then(|v| v.as_str())
            .unwrap_or("0x0");
        let gas_price = u64::from_str_radix(gas_price_hex.trim_start_matches("0x"), 16).unwrap_or(0);
    ...
    async fn handle_block_header(&self, block: &serde_json::Value) {
        let block_number = block.get("number")
            .and_then(|v| v.as_str())
...
pub fn default_config() -> EventLoopConfig {
    ...
    let ws_url = std::env::var("ARBITRUM_WS_URL")
        .unwrap_or_else(|_| ARB_WS_URLS[0].to_string());
